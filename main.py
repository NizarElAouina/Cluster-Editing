...
def set_triangular_col_to_zero(matrix, row_indexes):
    for i in row_indexes:
        #matrix[i, :i] = 0
        matrix[:, i] = 0
    return matrix
...
def make_cluster(matrix,filename):
    ...
    matrix = set_triangular_col_to_zero(matrix, numbers)
    result,sizes = find_squares(matrix)
    if result:
        return make_cluster(matrix, filename)
    else : 
        print("Done")
""" 
...
a=np.tril(a)
cl.make_cluster(a,"C:/Users/nizar/Desktop/S2/IA/Clustering/cluster3")
...
# Read in file and extract number of nodes and edges
with open('C:/Users/nizar/Desktop/S2/IA/Clustering/Protein.txt', 'r') as f:
    ...
    # Loop through edges and populate adjacency matrix
    for i in range(num_edges):
        ...
        if node1 not in node_dict:
            node_dict[node1] = len(node_dict)
        if node2 not in node_dict:
            node_dict[node2] = len(node_dict)
        node1_index = node_dict[node1]
        node2_index = node_dict[node2]
        adj_matrix[node1_index, node2_index] = 1
        adj_matrix[node2_index, node1_index] = 1  # Uncomment this line for an undirected graph
